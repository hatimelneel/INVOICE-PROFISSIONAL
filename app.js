...
setLang(lang);renderHome();populate();updateTotals();setTimeout(()=>initAutoDailyBackup(),0);
...
function addStatementPayment(){
  ...
  // Allocate to oldest unpaid invoices first. Partial allocation is allowed.
  invs.forEach(inv=>{
    ...
    const newPaid=Math.min((+inv.paid||0)+applied,+inv.total||0);
    inv.paid=Math.round(newPaid*1000)/1000;
    inv.balance=Math.max(Math.round(((+inv.total||0)-inv.paid)*1000)/1000,0);
    inv.paidFull=inv.balance<=0.0005;
  ...
  alert(tr(
    ...
    "تم تسجيل دفعة بمبلغ "+money(amount)+" بنجاح."
  ));
}
  ...
  if(document.getElementById("statementPaymentRows")){
    statementPaymentRows.innerHTML=payments.length ? payments.slice().reverse().map(p=>{
      ...
      return `<tr class="payment-history-row">
        <td>${new Date(p.date+"T00:00:00").toLocaleDateString(lang==="ar"?"ar-OM":"en-GB")}</td>
...
function buildDailyBackupPayload(backupForDate){
  return {backupForDate,app:"Invoice Pro",version:"2026.09.10-CLEAN-BACKUP",exportedAt:new Date().toISOString(),preferredCloud:localStorage.getItem("invoicepro_backup_destination")||"icloud",language:lang,data:safeBackupDB()};
}
...
async function exportCloudBackup(){
  const dest=localStorage.getItem("invoicepro_backup_destination")||"icloud";
  const payload={app:"Invoice Pro",version:"2026.09.10-CLEAN-BACKUP",exportedAt:new Date().toISOString(),preferredCloud:dest,language:lang,data:safeBackupDB()};
...
async function exportBackup(){
  const payload={app:"Invoice Pro",version:"2026.09.10-CLEAN-BACKUP",exportedAt:new Date().toISOString(),language:lang,data:safeBackupDB()};
